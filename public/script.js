document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function setError(field, message) {
      ...
      let errorEl = field.parentElement.querySelector('.field__error-message');
      if (!errorEl) {
    ...
    function clearError(field) {
      ...
      const errorEl = field.parentElement.querySelector('.field__error-message');
      if (errorEl) {
        errorEl.textContent = '';
      }
    }
    ...
    function validateField(field) {
      ...
      if (required && !value) {
        setError(field, field.dataset.errorRequired || 'This field is required.');
        return false;
      }

      if (value && type === 'email' && !isValidEmail(value)) {
        setError(field, field.dataset.errorEmail || 'Please enter a valid email address.');
        return false;
      }

      if (value && (type === 'tel' || name.toLowerCase().includes('phone')) && !isValidPhone(value)) {
        setError(field, field.dataset.errorPhone || 'Please enter a valid phone number.');
        return false;
      }

      const minLength = field.minLength > 0 ? field.minLength : null;
      if (minLength && value.length < minLength) {
        setError(field, field.dataset.errorMinlength || `Minimum ${minLength} characters required.`);
        return false;
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      const successMessage = contactForm.querySelector('.form__success-message');
      if (successMessage) {
  ...
  const ANIMATE_CLASS = 'fade-in';
  const VISIBLE_CLASS = 'fade-in--visible';
  const ANIMATE_SELECTOR = '.fade-in, [data-animate]';

  const animatedElements = document.querySelectorAll(ANIMATE_SELECTOR);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "RingYouth משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'dc1cea3d-219b-4611-b64d-7ca9a8ba3a64',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'dc1cea3d-219b-4611-b64d-7ca9a8ba3a64';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'dc1cea3d-219b-4611-b64d-7ca9a8ba3a64';
